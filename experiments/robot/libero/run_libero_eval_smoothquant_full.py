...
#   fp16          → 不量化 (baseline)
#   naive_w8a8    → 直接量化，不 smooth
#   smoothquant   → smooth + 量化
#
...
def initialize_model(cfg: GenerateConfig):
    ...
    # ===========================================================
    # SmoothQuant Fake Quantization (唯一改动在这里)
    # ===========================================================
    print(f"\n[SmoothQuant] QUANT_MODE = {QUANT_MODE}, QUANT_VISION = {QUANT_VISION}")
    ...
    if QUANT_MODE != "fp16":
        ...
        if QUANT_MODE == "smoothquant":
            # SmoothQuant: 先 smooth 再 quantize
        ...
        # naive_w8a8 或 smoothquant 都会走到这一步
        quantize_qwen(model)
        ...
        # 特殊情况: QUANT_MODE=fp16 但 QUANT_VISION=1
        # 只量化 ViT+Projector，不量化 Qwen2.5
        from smoothquant_vla_full import quantize_vision_projector
        quantize_vision_projector(model)
        print("[SmoothQuant] Only ViT + Projector quantized (Naive W8A8)")
    else:
        print("[SmoothQuant] Mode=fp16, no quantization applied")
    # ===========================================================
...
def setup_logging(cfg: GenerateConfig):
    ...
    # Create run ID (加入 QUANT_MODE 标识)
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    # Initialize model and components (SmoothQuant is applied inside here)
